...
import gaodeCitys from "../store/weather.js";
import {
  getConstellation,
  randomNum,
  sortBirthdayTime,
...
export const getWeather = async (city) => {
  ...
  if (cachedMap[city]) {
    return cachedMap[city];
  }
  // 找城市
  const theCity = gaodeCitys.find((item) => item["中文名"] === city);
  console.log("getWeather 2", theCity);

  if (!theCity) {
    throw new Error("找不到城市");
  }

  const url = `https://restapi.amap.com/v3/weather/weatherInfo?key=abd5f09a713d204e8ed7fb1f5bfaf2f3&city=${theCity.adcode}&extensions=all`;
  ...
  if (res.status === 200) {
    const info = res?.data?.forecasts?.[0]?.casts?.[0];
    cachedMap[city] = info;
    return info;
  }
...
export const getAggregatedData = async () => {
  ...
  for (const user of users) {
    ...
    const weatherInfo = await getWeather(useCity);
...
const sendMessageByWeChatTest = async (user, templateId, wxTemplateData) => {
  ...
  data.data.birthday_message.value =
    data.data.birthday_message.value.match(/还有(\d+)天/)?.[1] || 0;
  let loveWords = data.data.earthy_love_words.value.replace("。", "");
  data.data.love_word_1 = { value: loveWords.slice(0, 20) };
  data.data.love_word_2 = { value: loveWords.slice(20, 40) };
  data.data.love_word_3 = { value: loveWords.slice(40) };

  const oneTalks = data.data.one_talk.value.replace("。", "");
  data.data.one_talk_1 = { value: oneTalks.slice(0, 20) };
  data.data.one_talk_2 = { value: oneTalks.slice(20, 40) };
  data.data.one_talk_3 = { value: oneTalks.slice(40) };

  const daypower = data.data.daypower.value
    .replace("≤", "<=")
    .replace("≥", ">=");
  const nightpower = data.data.nightpower.value
    .replace("≤", "<=")
    .replace("≥", ">=");

  data.data.daypower = { value: daypower + "级" };
  data.data.nightpower = { value: nightpower + "级" };
